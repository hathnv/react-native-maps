...
const propTypes = {
  ...
  * custom marker press
  * */
  onCustomMarkerPress: PropTypes.func
};
...
class MapView extends React.Component {
  ...
  setMyLocationEnabled(bState) {
    this._runCommand('setMyLocationEnabled', [bState]);
  }
  ...
  addMarkerNative(latlng, imageSrc, pointId, rotation) {
    this._runCommand('addMarkerNative', [latlng, imageSrc, pointId, rotation]);
  }

  removeMarkerNative() {
    this._runCommand('removeMarkerNative', []);
  }

  addMarker(latlng, imageSrc, pointId, zoomLevel) {
    this._runCommand('addMarker', [latlng, imageSrc, pointId, zoomLevel]);
  }

  removeAllMarker() {
    this._runCommand('removeAllMarker', []);
  }
  ...
  zoomMap(zoom, duration) {
    this._runCommand('zoomMap', [zoom, duration || 500]);
  }
  // animateToBearingAndZoom(bearing, duration) {
  //   this._runCommand('animateToBearingAndZoom', [bearing, duration || 500]);
  // }
  animateToCameraWithZoomBearingAngle(latitude, longtidue, zoom, bearing, angle, duration) {
    this._runCommand('animateToCameraWithZoomBearingAngle', [latitude, longtidue, zoom, bearing, angle, duration || 500]);
  }
  animateToCameraWithZoomAngle(latitude, longtidue, zoom, angle, duration) {
    this._runCommand('animateToCameraWithZoomAngle', [latitude, longtidue, zoom, angle || 0, duration || 500]);
  }
  animateToCameraWithAngle(latitude, longtidue, zoom, angle, bearing, duration) {
    this._runCommand('animateToCameraWithAngle', [latitude, longtidue, zoom, angle, bearing, duration || 500]);
  }
  addMarkerNative(latlng, imageSrc, pointId, rotation) {
    this._runCommand('addMarkerNative', [latlng, imageSrc, pointId, rotation]);
  }
  ...
  pointForCoordinate(latlng) {
    this._runCommand('pointForCoordinate', [latlng]);
  }
